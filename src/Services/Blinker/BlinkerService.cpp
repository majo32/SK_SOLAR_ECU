    ...
    void BlinkerService::Listener_1Can_CU_STATE_Blinkers::handle(const SunStorm::Message& message) {
        ...
        if (messagesCU.btnLeftBlinker == 1 && messagesCU.btnWarningLights == 0) {
            ...
            if (ctx.counterLeftBlinker == 1) {
                ...
                ctx.intervalIdLongOrShortLEFT = this->RegisterInterval(100, this->GetService().listenerPressedButtomLongShort);
        ...
        if (messagesCU.btnLeftBlinker == 0 && messagesCU.btnWarningLights == 0) {
            ...
            this->RemoveInterval(ctx.intervalIdLongOrShortLEFT); //odstrani interval kt hovori ako dlho je stlacene tlacidlo
        ...
        if (messagesCU.btnRightBlinker == 1 && messagesCU.btnWarningLights == 0) {
            ...
            if (ctx.counterRightBlinker == 1) {
                ...
                ctx.intervalIdLongOrShortRight = this->RegisterInterval(100, this->GetService().listenerPressedButtomLongShort);
        ...
        if (messagesCU.btnWarningLights == 1) {
            ...
            if (ctx.helpValue == false) {
                ctx.intervalIdWarningLights = this->RegisterInterval(50, this->GetService().listenerSettingPeriod);
                ctx.helpValue = true;
            }
...
//        Messages::BlinkerServiceS::CAN_SC messagesSC;
        ...
        if (ctx.controlL == 1) {
            ...
            if (ctx.counterPeriod2 <= 5) {
                ctx.messagesSC.cmdLeftBlinker = 1;
                std::cout << "prva podmienka LAVA TESTOVANIE " << ctx.messagesSC.cmdLeftBlinker << std::endl;
                ctx.blinkerService.setSendContent(m, ctx.messagesSC);
            }
            if (ctx.counterPeriod2 > 5) {
                ctx.messagesSC.cmdLeftBlinker = 0;
                ctx.blinkerService.setSendContent(m, ctx.messagesSC);
                std::cout << ctx.counterPeriod2 << "druha podmienka LAVA TESTOVANIE " << ctx.messagesSC.cmdLeftBlinker << std::endl;
            }
        }
        if (ctx.controlR == 1) {
            if (ctx.counterPeriod2 <= 5) {
                ...
                ctx.blinkerService.setSendContent(m, ctx.messagesSC);
                std::cout << "prva podmienka PRAVA TESTOVANIE" << ctx.messagesSC.cmdRightBlinker << std::endl;
            }
            if (ctx.counterPeriod2 > 5) {
                ctx.messagesSC.cmdRightBlinker = 0;
                ctx.blinkerService.setSendContent(m, ctx.messagesSC);
                std::cout << ctx.counterPeriod2 << "druha podmienka PRAVA TESTOVANIE" << ctx.messagesSC.cmdRightBlinker << std::endl;
            }
        }
        if (ctx.controlW == 1) {
            if (ctx.counterPeriod2 <= 5) {
                ctx.messagesSC.cmdWarningLights = 1;
                ctx.blinkerService.setSendContent(m, ctx.messagesSC);
                std::cout << "prva podmienka VYSTRAZNE- TESTOVANIE " << ctx.messagesSC.cmdWarningLights << std::endl;
            }
            if (ctx.counterPeriod2 > 5) {
                ctx.messagesSC.cmdWarningLights = 0;
                ctx.blinkerService.setSendContent(m, ctx.messagesSC);
                std::cout << "druha podmienka VYSTRAZNE -TESTOVANIE " << ctx.messagesSC.cmdWarningLights << std::endl;
            }
        }
        ...
        if (ctx.counterPeriod2 == 10) {
            ...
            if ((ctx.counterPeriod1 == 3 && ctx.longShortPressed == 0)) {
                std::cout << "KRATKE STLACENIE VYPNUTIE- TESTOVANIE" << std::endl;
                ctx.messagesSC.cmdLeftBlinker = 0;
                ctx.messagesSC.cmdRightBlinker = 0;
                ctx.blinkerService.setSendContent(m, ctx.messagesSC);
                ctx.counterPeriod1 = 0;
            ...
            if ((ctx.counterPeriod1 >= 3 && ctx.longShortPressed == true && ctx.localStatusBtnLeft == 0 && ctx.localStatusBtnRight == 0)) {
                std::cout << " DLHE STLACENIE VYPNUTE -TESTOVANIE" << std::endl;
                this->RemoveInterval(ctx.intervalIdRight);
                this->RemoveInterval(ctx.intervalIdLeft);
                ctx.counterPeriod1 = 0;
                ctx.longShortPressed = false;
    ...
    void BlinkerService::setSendContent(SunStorm::Message & message, Messages::BlinkerServiceS::CAN_SC structure) {
